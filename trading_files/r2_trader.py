...
class OsmiumTrader(ProductTrader):
    ...
    SKEW_UNIT      = 20        # 1 tick of passive-quote skew per this many units of inventory
    ...
    def get_orders(self):
        ...
        #    When balanced, inventory-skew the passive quotes: shift both bid
        #    and ask toward the side that reduces inventory. Long → shift
        #    down (bid becomes less competitive, ask becomes more aggressive).
        #    Short → shift up. At SKEW_UNIT=20 the shift is mild (1 tick at
        #    pos=20, 2 ticks at pos=40), so edge is preserved while bias
        #    lightly rebalances fills. Kept conservative because prior rounds
        #    showed aggressive skew hurt PnL more than it helped.
        if self.position > self.FLATTEN_THRESH:
            ...
            skew = -int(self.position / self.SKEW_UNIT)
            if self.best_bid is not None and self.buy_capacity > 0:
                bid_price = min(self.best_bid + 1, fv - 1) + skew
                self.buy(bid_price, self.buy_capacity)
            if self.best_ask is not None and self.sell_capacity > 0:
                ask_price = max(self.best_ask - 1, fv + 1) + skew
                self.sell(ask_price, self.sell_capacity)
